...
### 🔧 Fixed
- **Video saving always grabbed the first post's video**: page-wide resource entries were tried before the clicked post's own `og:video`, so a blob-only (streamed) video resolved to the first mp4 the page had loaded. Candidates are now ordered post-scoped-first (element URL → the clicked post page's `og:video`/`og:image` → page-wide resources only as a last resort), and the post-link resolver prefers the status link that actually wraps the clicked media. Direct video downloads now target the right post — no Cobalt detour, much faster.
